...
export function ContingenciaTab({ companyId }: { companyId: string }) {
  ...
  useEffect(() => {
    async function load() {
      const [n, p, r, t] = await Promise.all([
        ...
        fetch(`/api/v1/admin/companies/${companyId}/traffic-authorities?page=1&pageSize=50`, {
          credentials: "include",
        }),
      ]);
      setNotifications(n ?? { channel: 0, notificationTarget: 1 });
      setPayments(p ?? { allowFlitGateway: false, allowOt: false, allowOther: false });
      setRunt(r ?? { primaryProvider: 0, secondaryProvider: 1, failoverTimeoutMs: 4000 });
      if (t.ok) {
  ...
  async function saveNotifications() {
    ...
    const target = targetOptions.find((o) => o.value === notifications.notificationTarget);
  ...
  async function toggleAuthority(code: string, enabled: boolean) {
    setSaving(true);
    const res = await fetch(`/api/v1/admin/companies/${companyId}/traffic-authorities`, {
      method: "PATCH",
    ...
    if (res.ok) {
      setTraffic((prev) =>
        prev.map((item) => (item.authorityCode === code ? { ...item, isEnabled: enabled } : item)),
      );
    ...
    <div className="space-y-8">
      <section className="space-y-3">
        ...
        <select
          ...
          onChange={(e) => setNotifications({ ...notifications, channel: Number(e.target.value) })}
        >
        ...
        {(
          ...
          <label key={key} className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) => setPayments({ ...payments, [key]: e.target.checked })}
            />
            {label}
          </label>
        ))}
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <select
            ...
            onChange={(e) => setRunt({ ...runt, primaryProvider: Number(e.target.value) })}
          >
          ...
          <select
            ...
            onChange={(e) => setRunt({ ...runt, secondaryProvider: Number(e.target.value) })}
          >
            {providerOptions.map((o) => (
              <option key={o.value} value={o.value}>
                Secundario: {o.label}
      ...
      <section>
        <h3 className="mb-3 text-sm font-semibold text-flit-text-brand">Organismos de tránsito</h3>
                ...
                <tr key={item.authorityCode} className="border-t border-flit-border-soft">
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      disabled={saving}
                      onChange={(e) => void toggleAuthority(item.authorityCode, e.target.checked)}
                    />
                  </td>
                </tr>
              ))}
