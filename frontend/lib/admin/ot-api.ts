import type { OtIndexSearchParams } from "./ot-types";

export function buildOtIndexQuery(params: OtIndexSearchParams): string {
  ...
  if (params.divipol?.trim()) query.set("divipol", params.divipol.trim());
...
export function otStatusLabel(status: number): string {
  return status === 1 ? "Suspendido" : "Activo";
}

export function otStatusApiValue(status: number): "Active" | "Suspended" {
